...
def insert_timestamps_on_frame_sequence_in_dir(
    ...
    for i, file_path in tqdm.tqdm(enumerate(list_of_files), total=len(list_of_files)):
        ...
        image_editor.insert_text_box(img=img,
        ...
        image_editor.insert_subtitle(img=img,
                                     text=frame_main_title,
                                     subtitle_height_percentage=0.1)

        image_editor.save_img(img=img,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tdir", nargs="?", help="Target dir to store new frames",
    ...
    try:
        ...
        fps = int(input(prRed("Warning, original frame rate was not found in filename"
